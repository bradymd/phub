...
async function main() {
  ...
  if (!masterPassword) {
    console.error('Usage: node attach-medical-documents.cjs <master-password>');
    process.exit(1);
  }
  ...
  // Get all files in medical directory
  const medicalFiles = fs.readdirSync(medicalDir);
  console.log(`Found ${medicalFiles.length} files in medical directory`);
  ...
  for (const record of records) {
    ...
    if (record.documents && record.documents.length > 0) {
      console.log(`Skipping "${record.condition}" - already has ${record.documents.length} documents attached`);
      skippedCount++;
      continue;
    }

    console.log(`\nProcessing: ${record.condition} (${record.date})`);
    console.log(`  Legacy attachments: ${record.attachments.join(', ')}`);

    record.documents = record.documents || [];

    // Try to match each attachment filename to actual files
    for (const attachmentFilename of record.attachments) {
      // Find matching file (case-insensitive)
      const matchingFile = medicalFiles.find(f =>
        f.toLowerCase() === attachmentFilename.toLowerCase()
      );

      if (matchingFile) {
        const filePath = path.join(medicalDir, matchingFile);
        ...
        console.log(`  ✗ Not found: ${attachmentFilename}`);
      }
    }
  }
